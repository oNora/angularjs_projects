...
export class QuoteService {
  ...
  generateRandomQuotes(count: number, delay: number, callback: (quote: Quote) => void) {
    this.generate(count, delay, callback);
  }

  private generate(remainingCount: number, delay:number, callback: (quote: Quote) => void){
    callback(this.getRandomQuote());
    if (remainingCount > 1) {
      setTimeout(() => this.generate(remainingCount - 1, delay, callback), delay);
    }
  }
}
